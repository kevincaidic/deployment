...
$("#activeUsersChart").sparkline([112,110,120,107,110,85,87,90,102,109,120,99,110,85,87,94], {
    type: 'bar',
    height: '700',
    barWidth: 9,
    barSpacing: 10,
...
// --- NEW CODE FOR ACTIVE SIDEBAR LINK ---
$(document).ready(function() {
    var currentPath = window.location.pathname; // Gets the path like "/dashboard.html"
    var currentFileName = currentPath.substring(currentPath.lastIndexOf('/') + 1); // Extracts "dashboard.html"

    // If the path is just "/" or empty, assume it's index.html or the dashboard.
    // Adjust this logic if your main page has a different name or path.
    if (currentFileName === "" || currentFileName === "/") {
        currentFileName = "index.html"; // Or whatever your default dashboard file is named
    }

    $(".sidebar .nav .nav-item a").each(function() {
        var linkHref = $(this).attr("href");
        if (linkHref === currentFileName) {
            // Remove active from any other .nav-item first (important for single-page templates or if re-applying)
            $(".sidebar .nav .nav-item").removeClass("active");

            // Add active class to the parent .nav-item of the matching link
            $(this).closest(".nav-item").addClass("active");

            // If it's a submenu item, also make its parent active
            var parentCollapse = $(this).closest(".collapse");
            if (parentCollapse.length) {
                parentCollapse.addClass("show"); // Show the collapsed menu
                parentCollapse.prev(".nav-item").addClass("active"); // Add active to the parent nav-item
            }
        }
    });
});
